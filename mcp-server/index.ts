 ...
 * Clarity MCP Server
 *
 * Exposes Clarity project tools to Claude Desktop and other MCP clients.
 * Communicates with the Clarity API via REST endpoints.
 *
 * Usage:
 *   CLARITY_API_KEY=<key> CLARITY_API_URL=<url> npx tsx mcp-server/index.ts
 */
...
const API_URL = process.env.CLARITY_API_URL || 'https://claritynotes.xyz'
const API_KEY = process.env.CLARITY_API_KEY || ''

if (!API_KEY) {
  console.error('CLARITY_API_KEY is required. Generate one in Clarity Settings.')
  process.exit(1)
}
...
OUTPUT: Generate the complete TikZ code ready to be inserted into the user's document or a new file.`,
  ...
  async ({ project_id, description, file_name }) => {
    // This tool's real work is done by the LLM — we provide context and structure.
    // The tool creates the file if requested, or returns instructions.
    ...
    if (file_name) {
      // Create the file with a placeholder, then the LLM will write_file with actual content
      try {
        const data = await api(`/projects/${project_id}/files`, {
          method: 'POST',
          body: JSON.stringify({
