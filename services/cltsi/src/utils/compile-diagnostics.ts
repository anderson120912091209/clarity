...
export async function buildCompileDiagnostics(
  options: DiagnosticOptions
): Promise<CompileDiagnostics> {
...
async function readOutputLog(compileDir: string): Promise<string | null> {
  try {
...
function extractLatexDiagnostic(logs: string | null): CompileDiagnostics | null {
  if (!logs) return null;
...
function extractTypstDiagnostic(logs: string | null): CompileDiagnostics | null {
  if (!logs) return null;
...
function summarizeInfrastructureMessage(message: string): string {
